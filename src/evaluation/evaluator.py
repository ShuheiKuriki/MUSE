...
class Evaluator():
    ...
    def monolingual_wordsim(self, to_log):
        ...
        src_ws_scores = get_wordsim_scores(
            ...
            self.generator(self.src_emb.weight).detach().cpu().numpy()
        )
    ...
    def crosslingual_wordsim(self, to_log):
        ...
        """
        src_emb = self.generator(self.src_emb.weight).detach().cpu().numpy()
    ...
    def eval_dis(self, to_log):
        ...
        with no_grad():
            for i in range(0, self.src_emb.num_embeddings, bs):
                preds = self.discriminator(self.generator(self.src_emb.weight[i:i + bs].detach()))
