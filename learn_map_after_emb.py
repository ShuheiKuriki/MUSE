...
parser.add_argument("--dis_sampling_emb", type=float, default=5, help="probality of learning discriminator")
parser.add_argument("--dis_sampling_map", type=float, default=5, help="probality of learning discriminator")
...
parser.add_argument("--n_epochs_emb", type=int, default=20, help="Number of epochs")
parser.add_argument("--n_epochs_map", type=int, default=15, help="Number of epochs")
...
if params.adversarial:
    ...
    trainer.reload(params.map_path)
    to_log = OrderedDict()
    evaluator.all_eval(to_log)
    evaluator.eval_dis(to_log)

    # emb training loop
    for n_epoch in range(params.n_epochs_emb):
        logger.info('Starting emb training epoch %i...', n_epoch)
        tic = time.time()
        n_words_proc = 0
        stats = {'DIS_COSTS': []}
        ...
        for n_iter in range(0, params.epoch_size, params.batch_size):
            ...
            if params.dis_sampling_emb < 1:
                if np.random.rand() <= params.dis_sampling_emb:
                    trainer.dis_step(stats)
            else:
                for i in range(int(params.dis_sampling_emb)):
                    trainer.dis_step(stats)
    ...
    # map training loop
    for n_epoch in range(params.n_epochs_map):
        ...
        for n_iter in range(0, params.epoch_size, params.batch_size):
            ...
            # discriminator training
            if params.dis_sampling_map < 1:
                if np.random.rand() <= params.dis_sampling_map:
                    trainer.dis_step(stats)
            else:
                for i in range(int(params.dis_sampling_map)):
                    trainer.dis_step(stats)
