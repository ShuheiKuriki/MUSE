...
* learn_univ_w_init_emb.py : 普遍空間の初期値を別ファイルから取り込み，普遍空間の学習を行う．初期値を事前に学習しておかないと生けまい．

初期値を学習するコードはMAT(Multilingual Adversarial Training)の導入に対応できていなさそうなのでまだ公開していません
